...
import Agda.Core.ToCore (convert, ToCoreGlobal(..), Data(..), Constructor(..))
...
import Agda.Core.Checkers.TerminateFind (checkTerminationFind)
  ...
  for_ (zip (iterate Scope.inThere Scope.inHere) defs) $ \(i, def) -> do
    case def of
      ...
      Right Core.Definition{ defName, theDef = Core.FunctionDefn funBody, defType } -> do
        ...
        case checkTerminationFind [] [] i funBody of
